...
public class JobHistoryFileParserHadoop2 implements JobHistoryFileParser {
	...
	public static enum Hadoop2RecordType {
		...
		JobPriorityChange, JobStatusChanged, JobSubmitted,
		JobUnsuccessfulCompletion, MapAttemptFinished,
		ReduceAttemptFinished, TaskAttemptFinished,
		TaskAttemptStarted, TaskAttemptUnsuccessfulCompletion,
    ...
    /**
     * constructor
     * populates the counterNames hash set and the fieldTypes map
     * 
     */
    JobHistoryFileParserHadoop2() {
    	/**
    	 * populating this map since the symbols and
    	 * key to get the types of fields
    	 *
    	 * the symbol denotes the record in the file (like JOB_SUBMITTED)
    	 * and it's value in the map (like JobSubmitted) helps us get the
    	 * types of fields that that record contains
    	 * (this type information is present in the schema)
    	 *
    	 */
    	...
    	eventRecordNames.put("CLEANUP_ATTEMPT_KILLED", Hadoop2RecordType.TaskAttemptUnsuccessfulCompletion);
    	eventRecordNames.put("CLEANUP_ATTEMPT_FAILED", Hadoop2RecordType.TaskAttemptUnsuccessfulCompletion);
    	...
    	eventRecordNames.put("SETUP_ATTEMPT_KILLED", Hadoop2RecordType.TaskAttemptUnsuccessfulCompletion);
    	eventRecordNames.put("SETUP_ATTEMPT_FAILED", Hadoop2RecordType.TaskAttemptUnsuccessfulCompletion);
    	eventRecordNames.put("SETUP_ATTEMPT_STARTED", Hadoop2RecordType.TaskAttemptStarted);
    	eventRecordNames.put("REDUCE_ATTEMPT_KILLED", Hadoop2RecordType.TaskAttemptUnsuccessfulCompletion);
    	eventRecordNames.put("REDUCE_ATTEMPT_FAILED", Hadoop2RecordType.TaskAttemptUnsuccessfulCompletion);
    	...
    	counterNames.add(CounterTypes.counters.toString());
    	counterNames.add(CounterTypes.reduceCounters.toString());
    	counterNames.add(CounterTypes.totalCounters.toString());
    }
	...
	public void parse(String historyFileContents, JobKey jobKey)
		...
		try {
			FSDataInputStream in = new FSDataInputStream(new ByteArrayWrapper(historyFileContents.getBytes()));
	 ...
	 * so that we can parse the rest of the file
				...
				if (name.toString().equalsIgnoreCase(EVENT)) {
					...
					for (int i = 0; i < allTypeDetails.length(); i++) {
						...
						for (int j = 0; j < types.length(); j++) {
							typeDetails.put(
									types.getJSONObject(j).getString(NAME),
									types.getJSONObject(j).getString(TYPE));
						}
						fieldTypes.put(Hadoop2RecordType.valueOf(actual.get(
								NAME).toString()), typeDetails);
					}
				}
			}
		}
	...
	/**
	 * process the counter details
	 * example line in .jhist file for counters:
		 {
		  "name":"MAP_COUNTERS",
		  "groups":[
		    {
		      ...
		      "counts":[
		        {
		          ...
		          "displayName":"HDFS: Number of bytes read",
		          "value":480
		        },
		        {
		          "name":"HDFS_BYTES_WRITTEN",
		          "displayName":"HDFS: Number of bytes written",
		          "value":0
		        }
		      ]
		    },
		...
		try {
			...
			for (int i = 0; i < groups.length(); i++) {
				...
				for (int j = 0; j < counts.length(); j++) {
					...
					populatePut(p, Constants.INFO_FAM_BYTES,
							counterMetaGroupName,
							aCounter.get(NAME).toString(),
							countDetails.get(NAME).toString(),
							countDetails.getLong(VALUE));
				}
			}
		} catch (JSONException e) {
			throw new ProcessingException(
					" Caught json exception while processing counters ", e);
		}
	 ...
	 * @throws JSONException
	 */
	private void iterateAndPreparePuts(JSONObject eventDetails, Put p,
			Hadoop2RecordType recType) throws JSONException {
	...
	private void processRecords(Hadoop2RecordType recType, JSONObject eventDetails)
		...
		case AMStarted:
			byte[] amAttemptIdKeyBytes = getAMKey(AM_ATTEMPT_PREFIX,
					eventDetails.getString(APPLICATION_ATTEMPTID));
		...
		case MapAttemptFinished:
			byte[] taskMAttemptIdKeyBytes = getTaskKey(TASK_ATTEMPT_PREFIX,
					this.jobNumber, eventDetails.getString(ATTEMPTID));
			Put pMTaskAttempt = new Put(taskMAttemptIdKeyBytes);
			pMTaskAttempt.add(Constants.INFO_FAM_BYTES,
					Constants.RECORD_TYPE_COL_BYTES,
		...
		case ReduceAttemptFinished:
			byte[] taskRAttemptIdKeyBytes = getTaskKey(TASK_ATTEMPT_PREFIX,
					this.jobNumber, eventDetails.getString(ATTEMPTID));
			Put pRTaskAttempt = new Put(taskRAttemptIdKeyBytes);
			pRTaskAttempt.add(Constants.INFO_FAM_BYTES,
					Constants.RECORD_TYPE_COL_BYTES,
		...
		case TaskAttemptUnsuccessfulCompletion:
			byte[] taskAttemptIdKeyBytes = getTaskKey(TASK_ATTEMPT_PREFIX,
					this.jobNumber, eventDetails.getString(ATTEMPTID));
		...
		case TaskUpdated:
		case TaskFinished:
			byte[] taskIdKeyBytes = getTaskKey(TASK_PREFIX, this.jobNumber,
					eventDetails.getString(TASKID));
		...
		default:
			...
			throw new ProcessingException(
					"Check if recType was modified and has new members? "
							+ recType);
		}
	}
	...
	private void populatePut(Put p, byte[] family, String key,
		...
		byte[] qualifier = Bytes.toBytes(key.toLowerCase());
	...
	private void populatePut(Put p, byte[] family, String key,
		...
		byte[] groupPrefix = Bytes.add(counterPrefix, Bytes.toBytes(groupName),
				Constants.SEP_BYTES);
