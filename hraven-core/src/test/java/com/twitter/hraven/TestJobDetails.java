...
/**
 * Unit tests for functions in the {@link JobDetails} class
 * specifically for the populate function
 * to enable returning different counter sub group names that
 * changed between hadoop 1 and hadoop 2
 * 
 */
public class TestJobDetails {
	...
	private void addInMapCounters(NavigableMap<byte[], byte[]> infoValues) {
		...
		// hadoop1 counters
		byte[] c1 = Bytes.add(Constants.MAP_COUNTER_COLUMN_PREFIX_BYTES,
				Constants.SEP_BYTES);
		byte[] c2 = Bytes.add(c1,
				Bytes.toBytes(Constants.FILESYSTEM_COUNTERS.toString()));
		byte[] c3 = Bytes.add(c2, Constants.SEP_BYTES);

		byte[] c4 = Bytes.add(c3,
				Bytes.toBytes(Constants.FILES_BYTES_READ.toString()));

		byte[] value = Bytes.toBytes(JobDetailsValues.FILES_BYTES_READ_2);
		infoValues.put(c4, value);

		c4 = Bytes.add(c3,
				Bytes.toBytes(Constants.FILES_BYTES_WRITTEN.toString()));
		infoValues.put(c4, Bytes.toBytes(JobDetailsValues.FILES_BYTES_WRITTEN_2));

		// hadoop2 counters
		c1 = Bytes.add(Constants.MAP_COUNTER_COLUMN_PREFIX_BYTES,
				Constants.SEP_BYTES);
		c2 = Bytes.add(c1,
				Bytes.toBytes(Constants.FILESYSTEM_COUNTER_HADOOP2.toString()));
	...
	private void addInReduceCounters(NavigableMap<byte[], byte[]> infoValues) {
		...
		// hadoop 1 counters 
		byte[] c1 = Bytes.add(Constants.REDUCE_COUNTER_COLUMN_PREFIX_BYTES,
				Constants.SEP_BYTES);
		byte[] c2 = Bytes.add(c1,
				Bytes.toBytes(Constants.FILESYSTEM_COUNTERS.toString()));
		byte[] c3 = Bytes.add(c2, Constants.SEP_BYTES);
		...
		c2 = Bytes.add(c1,
				Bytes.toBytes(Constants.TASK_COUNTER.toString()));
		c3 = Bytes.add(c2, Constants.SEP_BYTES);
		c4 = Bytes.add(c3,
				Bytes.toBytes(Constants.REDUCE_SHUFFLE_BYTES.toString()));
		infoValues.put(c4, Bytes.toBytes(JobDetailsValues.REDUCE_SHUFFLE_BYTES_2));

		// hadoop2 counters
		c1 = Bytes.add(Constants.REDUCE_COUNTER_COLUMN_PREFIX_BYTES,
				Constants.SEP_BYTES);
		c2 = Bytes.add(c1,
				Bytes.toBytes(Constants.FILESYSTEM_COUNTER_HADOOP2.toString()));
		c3 = Bytes.add(c2, Constants.SEP_BYTES);
		...
		c2 = Bytes.add(c1,
				Bytes.toBytes(Constants.TASK_COUNTER_HADOOP2.toString()));
		c3 = Bytes.add(c2, Constants.SEP_BYTES);
		c4 = Bytes.add(c3,
				Bytes.toBytes(Constants.REDUCE_SHUFFLE_BYTES.toString()));
		infoValues.put(c4, Bytes.toBytes(JobDetailsValues.REDUCE_SHUFFLE_BYTES_2));
	...
	private void addInTotalCounters(NavigableMap<byte[], byte[]> infoValues) {
		...
		// hadoop 1 counters
		byte[] c1 = Bytes.add(Constants.COUNTER_COLUMN_PREFIX_BYTES,
				Constants.SEP_BYTES);
		byte[] c2 = Bytes.add(c1,
				Bytes.toBytes(Constants.FILESYSTEM_COUNTERS.toString()));
		byte[] c3 = Bytes.add(c2, Constants.SEP_BYTES);
		...
		byte[] c4 = Bytes.add(c3,
				Bytes.toBytes(Constants.HDFS_BYTES_READ.toString()));
		byte[] value = Bytes.toBytes(JobDetailsValues.HDFS_BYTES_READ_2);
		infoValues.put(c4, value);

		c4 = Bytes.add(c3,
				Bytes.toBytes(Constants.HDFS_BYTES_WRITTEN.toString()));
		value = Bytes.toBytes(JobDetailsValues.HDFS_BYTES_WRITTEN_2);
		infoValues.put(c4, value);

		c2 = Bytes.add(c1,
				Bytes.toBytes(Constants.JOBINPROGRESS_COUNTER.toString()));
		c3 = Bytes.add(c2, Constants.SEP_BYTES);
		c4 = Bytes.add(c3,
				Bytes.toBytes(Constants.SLOTS_MILLIS_MAPS.toString()));
		value = Bytes.toBytes(JobDetailsValues.SLOTS_MILLIS_MAPS_2);
		infoValues.put(c4, value);

		c4 = Bytes.add(c3,
				Bytes.toBytes(Constants.SLOTS_MILLIS_REDUCES.toString()));
		value = Bytes.toBytes(JobDetailsValues.SLOTS_MILLIS_REDUCES_2);
		infoValues.put(c4, value);

		// hadoop 2 counters
		c1 = Bytes.add(Constants.COUNTER_COLUMN_PREFIX_BYTES,
				Constants.SEP_BYTES);
		c2 = Bytes.add(c1,
				Bytes.toBytes(Constants.FILESYSTEM_COUNTER_HADOOP2.toString()));
		c3 = Bytes.add(c2, Constants.SEP_BYTES);
		...
		c2 = Bytes.add(c1,
				Bytes.toBytes(Constants.JOB_COUNTER_HADOOP2.toString()));
		c3 = Bytes.add(c2, Constants.SEP_BYTES);
		c4 = Bytes.add(c3,
				Bytes.toBytes(Constants.SLOTS_MILLIS_MAPS.toString()));
		value = Bytes.toBytes(JobDetailsValues.SLOTS_MILLIS_MAPS_2);
	...
	private NavigableMap<byte[], byte[]> getInfoValues(Constants.HADOOP_VERSION hv) {
		NavigableMap<byte[], byte[]> infoValues = new TreeMap<byte[], byte[]>(
		...
		infoValues.put(JobHistoryKeys.KEYS_TO_BYTES
				.get(JobHistoryKeys.FINISHED_REDUCES), Bytes.toBytes(JobDetailsValues.finishedReduces));
		infoValues.put(JobHistoryKeys.KEYS_TO_BYTES.get(JobHistoryKeys.FAILED_MAPS),
				Bytes.toBytes(JobDetailsValues.failedMaps));
		infoValues.put(JobHistoryKeys.KEYS_TO_BYTES
						.get(JobHistoryKeys.FAILED_REDUCES), Bytes.toBytes(JobDetailsValues.failedReduces));
		infoValues.put(JobHistoryKeys.KEYS_TO_BYTES.get(JobHistoryKeys.FINISHED_MAPS),
				Bytes.toBytes(JobDetailsValues.finishedMaps));
	...
	private void confirmHadoop2Counters(JobDetails jd) {
		assertEquals(JobDetailsValues.FILES_BYTES_READ_2, 
				jd.getMapCounters().getCounter(Constants.FILESYSTEM_COUNTER_HADOOP2,
						Constants.FILES_BYTES_READ).getValue());
		
		assertEquals(JobDetailsValues.FILES_BYTES_WRITTEN_2, 
				jd.getMapCounters().getCounter(Constants.FILESYSTEM_COUNTER_HADOOP2,
						Constants.FILES_BYTES_WRITTEN).getValue());
		
		assertEquals(JobDetailsValues.FILES_BYTES_READ_2, 
				jd.getReduceCounters().getCounter(Constants.FILESYSTEM_COUNTER_HADOOP2,
						Constants.FILES_BYTES_READ).getValue());

		assertEquals(JobDetailsValues.REDUCE_SHUFFLE_BYTES_2, 
				jd.getReduceCounters().getCounter(Constants.TASK_COUNTER_HADOOP2,
						Constants.REDUCE_SHUFFLE_BYTES).getValue());

		assertEquals(JobDetailsValues.HDFS_BYTES_READ_2, 
				jd.getCounters().getCounter(Constants.FILESYSTEM_COUNTER_HADOOP2,
						Constants.HDFS_BYTES_READ).getValue());

		assertEquals(JobDetailsValues.HDFS_BYTES_WRITTEN_2, 
				jd.getCounters().getCounter(Constants.FILESYSTEM_COUNTER_HADOOP2,
						Constants.HDFS_BYTES_WRITTEN).getValue());

		assertEquals(JobDetailsValues.SLOTS_MILLIS_MAPS_2,
				jd.getCounters().getCounter(Constants.JOB_COUNTER_HADOOP2,
						Constants.SLOTS_MILLIS_MAPS).getValue());

		assertEquals(JobDetailsValues.SLOTS_MILLIS_REDUCES_2, 
				jd.getCounters().getCounter(Constants.JOB_COUNTER_HADOOP2,
						Constants.SLOTS_MILLIS_REDUCES).getValue());		
	}

	private void confirmHadoop1Counters(JobDetails jd) {
		assertEquals(JobDetailsValues.FILES_BYTES_READ_1, 
				jd.getMapCounters().getCounter(Constants.FILESYSTEM_COUNTERS,
						Constants.FILES_BYTES_READ).getValue());
		...
		assertEquals(JobDetailsValues.FILES_BYTES_READ_1, 
				jd.getReduceCounters().getCounter(Constants.FILESYSTEM_COUNTERS,
						Constants.FILES_BYTES_READ).getValue());

		assertEquals(JobDetailsValues.REDUCE_SHUFFLE_BYTES_1, 
				jd.getReduceCounters().getCounter(Constants.TASK_COUNTER,
						Constants.REDUCE_SHUFFLE_BYTES).getValue());

		assertEquals(JobDetailsValues.HDFS_BYTES_READ_1,
				jd.getCounters().getCounter(Constants.FILESYSTEM_COUNTERS,
						Constants.HDFS_BYTES_READ).getValue());
		...
		assertEquals(JobDetailsValues.SLOTS_MILLIS_MAPS_1,
				jd.getCounters().getCounter(Constants.JOBINPROGRESS_COUNTER,
						Constants.SLOTS_MILLIS_MAPS).getValue());
	...
	private void confirmHadoopVersion(JobDetails jd, Constants.HADOOP_VERSION hv) {
		assertEquals(hv.toString(), jd.getHadoopVersion());
	}
	...
	public void testPopulateForHadoop2() {
		...
		when(result.getFamilyMap(Constants.INFO_FAM_BYTES)).thenReturn(
				infoValues);
	...
	public void testPopulateForHadoop1() {
		...
		when(result1.getFamilyMap(Constants.INFO_FAM_BYTES)).thenReturn(
				infoValues);
