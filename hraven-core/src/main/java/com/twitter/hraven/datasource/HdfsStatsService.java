...
/**
 * Service that accesses the hdfs stats tables and populates the HdfsStats
 * object
 */
public class HdfsStatsService {
   ...
   * @throws IOException
   */
  public HdfsStatsService(Configuration hbaseConf, Connection hbaseConnection)
      throws IOException {
    this.hbaseConnection = hbaseConnection;

    defaultScannerCaching =
        hbaseConf.getInt("hbase.client.scanner.caching", 100);
    LOG.info(" in HdfsStatsService constuctor for "
        + HdfsConstants.HDFS_USAGE_TABLE);
  ...
  private Scan createScanWithAllColumns() {
    ...
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.FILE_COUNT_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.DIR_COUNT_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.ACCESS_COUNT_TOTAL_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.ACCESS_COST_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.STORAGE_COST_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.SPACE_CONSUMED_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.TMP_FILE_COUNT_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.TMP_SPACE_CONSUMED_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.TRASH_FILE_COUNT_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.TRASH_SPACE_CONSUMED_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.OWNER_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
        HdfsConstants.QUOTA_COLUMN_BYTES);
    scan.addColumn(HdfsConstants.DISK_INFO_FAM_BYTES,
   ...
   */
  private int populateHdfsStats(Result result,
    ...
    if (checkPath) {
      ...
      // sanity check
      if ((currentFullKey.getRunId() < endtime)
          || (currentFullKey.getRunId() > starttime)) {
        return rowCount;
      }
    }
  ...
  /**
   * this function returns an older timestamp ideally we would be using the
   * daily aggregation table and be simply looking for last run of the
   * aggregation, but right now, in the hourly table, we try to look for certain
   * timestamps in the past instead of scanning the entire table for the last
   * run.
  ...
  public static long getOlderRunId(int i, long runId) {
    ...
    /**
     * consider the day before old date and add randomizedHour for example, for
     * retryCount = 0 pick a day which is 2 days before ageMult[0], that is 2
     * days before the current date in consideration and add the randomized hour
     * say 13 hours, so that gives an hour belonging to one day before current
     * date in consideration
     */
    if (i >= HdfsConstants.ageMult.length) {
      throw new ProcessingException("Can't look back in time that far " + i
          + ", only upto " + HdfsConstants.ageMult.length);
    }
    long newTs =
        runId - (HdfsConstants.ageMult[i] * HdfsConstants.NUM_SECONDS_IN_A_DAY
            + randomizedHourInSeconds);
    LOG.info(" Getting older runId for " + runId + ", returning " + newTs
        + " since ageMult[" + i + "]=" + HdfsConstants.ageMult[i]
        + " randomizedHourInSeconds=" + randomizedHourInSeconds);
    return newTs;
