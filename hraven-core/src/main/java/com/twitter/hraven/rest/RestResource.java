...
import static com.twitter.hraven.rest.RestJSONResource.SLASH;
...
/**
 * Main REST resource that handles binding the REST API to the various services.
 * Do not use this class in clients, it will cause you to be connected to HBase.
 *
 * TODO: better prevalidation TODO: handle null results with empty json object
 * or response code
 */
@Path("/api/v1/")
public class RestResource {
  ...
  // TODO use regular lifecycle to create and close connection
  private static final Connection HBASE_CONNECTION = connectToHBase(HBASE_CONF);

  /**
   * @param hbaseConf used to connect to HBase
   * @return connection to HBase, or throw a RuntimeException if an IOException
   *         occurs.
   */
  private static Connection connectToHBase(Configuration hbaseConf) {
    try {
      return ConnectionFactory.createConnection(hbaseConf);
    } catch (IOException ioe) {
      LOG.fatal("Failed to connect to HBase", ioe);
      throw new RuntimeException("Failed to connect to HBase", ioe);
    }
  }
  ...
  public static final ThreadLocal<SerializationContext> serializationContext =
      new ThreadLocal<SerializationContext>() {
        ...
        protected SerializationContext initialValue() {
          // by default all retrieved data is serialized, each end point can
          // override
  ...
  public Flow getJobFlowById(@PathParam("cluster") String cluster,
      ...
      throws IOException {
    LOG.info(String.format("Fetching Flow for cluster=%s, jobId=%s", cluster,
        jobId));
    Stopwatch timer = new Stopwatch().start();
    ...
    JobHistoryService jobHistoryService =
        new JobHistoryService(HBASE_CONF, HBASE_CONNECTION);
    Flow flow = jobHistoryService.getFlowByJobID(cluster, jobId, false);
    timer.stop();
    ...
    if (flow != null) {
      LOG.info("For jobFlow/{cluster}/{jobId} with input query: " + "jobFlow/"
          ...
          + " fetched flow " + flow.getFlowName() + " with # "
          + flow.getJobCount() + " in " + timer);
    } else {
      LOG.info("For jobFlow/{cluster}/{jobId} with input query: " + "jobFlow/"
          + cluster + SLASH + jobId + "&"
          + StringUtil.buildParam("includeJobField", includeJobFields) + "&"
  ...
  public List<Flow> getJobFlowById(@PathParam("cluster") String cluster,
      @PathParam("user") String user, @PathParam("appId") String appId,
      @PathParam("version") String version, @QueryParam("limit") int limit,
    ...
    if (includeConfig != null && !includeConfig.isEmpty()) {
      ...
      configFilter =
          new SerializationContext.RegexConfigurationFilter(includeConfigRegex);
    }
    ...
    serializationContext.set(
        new SerializationContext(SerializationContext.DetailLevel.EVERYTHING,
            configFilter, flowFilter, jobFilter, null));
    List<Flow> flows =
        getFlowList(cluster, user, appId, version, startTime, endTime, limit);
    timer.stop();
    ...
    if (flows != null) {
      LOG.info("For flow/{cluster}/{user}/{appId}/{version} with input query: "
          ...
          + "?limit=" + limit + " startTime=" + startTime + " endTime="
          + endTime + " &includeConf=" + builderIncludeConfigs
          + " &includeConfRegex=" + builderIncludeConfigRegex
          + StringUtil.buildParam("includeJobField", includeJobFields) + "&"
          + StringUtil.buildParam("include", include) + " fetched "
  ...
  public List<Flow> getJobFlowById(@PathParam("cluster") String cluster,
      @PathParam("user") String user, @PathParam("appId") String appId,
      @QueryParam("limit") int limit, @QueryParam("startTime") long startTime,
      @QueryParam("endTime") long endTime,
      @QueryParam("include") List<String> include,
      @QueryParam("includeConf") List<String> includeConfig,
      @QueryParam("includeConfRegex") List<String> includeConfigRegex,
    ...
    List<Flow> flows =
        getFlowList(cluster, user, appId, null, startTime, endTime, limit);
    timer.stop();
    ...
    if (flows != null) {
      LOG.info("For flow/{cluster}/{user}/{appId} with input query: " + "flow/"
          + cluster + SLASH + user + SLASH + appId + "?limit=" + limit
          + "&startTime=" + startTime + "&endTime=" + endTime + "&includeConf="
          + builderIncludeConfigs + "&includeConfRegex="
  ...
  public PaginatedResult<Flow> getJobFlowStats(
      @PathParam("cluster") String cluster, @PathParam("user") String user,
      @PathParam("appId") String appId, @QueryParam("version") String version,
    ...
    LOG.info(
        "Fetching flowStats for flowStats/{cluster}/{user}/{appId} with input query: "
            + "flowStats/" + cluster + SLASH + " user " + user + appId
            + "?version=" + version + "&limit=" + limit + "&startRow="
            + startRowParam + "&startTime=" + startTime + "&endTime=" + endTime
    ...
    JobHistoryService jobHistoryService =
        new JobHistoryService(HBASE_CONF, HBASE_CONNECTION);
    List<Flow> flows = jobHistoryService.getFlowTimeSeriesStats(cluster, user,
    ...
    if (flows.size() > limit) {
      ...
      flowStatsPage.setNextStartRow(
          new FlowKeyConverter().toBytes(flows.get(limit).getFlowKey()));
    } else {
    ...
    LOG.info("For flowStats/{cluster}/{user}/{appId} with input query: "
        + "flowStats/" + cluster + SLASH // + user /{appId} cluster + " user " +
                                         // user
        + appId + "?version=" + version + "&limit=" + limit + "&startRow="
        + startRow + "&startTime=" + startTime + "&endTime=" + endTime
        + "&includeJobs=" + includeJobs + "&"
        + StringUtil.buildParam("includeJobField", includeJobFields)
  ...
  @Produces(MediaType.APPLICATION_JSON)
  public List<VersionInfo> getDistinctVersions(
      @PathParam("cluster") String cluster, @PathParam("user") String user,
    ...
    if (LOG.isTraceEnabled()) {
      LOG.trace("Fetching App Versions for cluster=" + cluster + " user=" + user
          + " app=" + appId);
    }
    serializationContext.set(
        new SerializationContext(SerializationContext.DetailLevel.EVERYTHING));

    AppVersionService appVersionService = new AppVersionService(HBASE_CONNECTION);

    List<VersionInfo> distinctVersions =
        appVersionService.getDistinctVersions(StringUtils.trimToEmpty(cluster),
            StringUtils.trimToEmpty(user), StringUtils.trimToEmpty(appId));
    timer.stop();

    LOG.info("For appVersion/{cluster}/{user}/{appId}/ with input query "
        + "appVersion/" + cluster + SLASH + user + SLASH + appId + "?limit="
        + limit + " fetched #number of VersionInfo " + distinctVersions.size()
        + " in ");// + timer);
  ...
  public String getCluster(@QueryParam("hostname") String hostname)
    ...
    if (StringUtils.isBlank(hostname)) {
      throw new ProcessingException(
          "hostname not present, " + "please resend with that info");
    }
    String cluster = Cluster.getIdentifier(hostname);
    LOG.info(
        "Fetched cluster identifier=" + cluster + " for hostname=" + hostname);
    return cluster;
  }

  private List<Flow> getFlowList(String cluster, String user, String appId,
      String version, long startTime, long endTime, int limit)
      throws IOException {
    if (limit < 1) {
      limit = 1;
    }
    ...
    JobHistoryService jobHistoryService =
        new JobHistoryService(HBASE_CONF, HBASE_CONNECTION);
    List<Flow> flows = jobHistoryService.getFlowSeries(cluster, user, appId,
        version, false, startTime, endTime, limit);
  ...
  public List<HdfsStats> getHdfsStats(@PathParam("cluster") String cluster,
    ...
    serializationContext.set(
        new SerializationContext(SerializationContext.DetailLevel.EVERYTHING));
    HdfsStatsService hdfsStatsService = new HdfsStatsService(HBASE_CONF, HBASE_CONNECTION);
    List<HdfsStats> hdfsStats =
        hdfsStatsService.getAllDirs(cluster, pathPrefix, limit, runid);
    timer.stop();
    /**
     * if we find NO hdfs stats for the default timestamp consider the case when
     * no runId is passed in that means user is expecting a default response we
     * set the default runId to 2 hours back as above but what if there was an
     * error in collection at that time? hence we try to look back for some
     * older runIds
     */
  ...
  public List<HdfsStats> getHdfsPathTimeSeriesStats(
      @PathParam("cluster") String cluster, @QueryParam("path") String path,
      @QueryParam("starttime") long starttime,
      @QueryParam("endtime") long endtime, @QueryParam("limit") int limit)
      throws IOException {
    ...
    if (endtime > starttime) {
      throw new RuntimeException("Ensure endtime " + endtime
          + " is older than starttime " + starttime);
    }

    LOG.info(String.format(
        "Fetching hdfs timeseries stats for cluster=%s, path=%s limit=%d, starttime=%d endtime=%d",
        cluster, path, limit, starttime, endtime));
    Stopwatch timer = new Stopwatch().start();
    List<HdfsStats> hdfsStats = new HdfsStatsService(HBASE_CONF, HBASE_CONNECTION)
        .getHdfsTimeSeriesStats(cluster, path, limit, starttime, endtime);
    timer.stop();

    if (hdfsStats != null) {
      LOG.info("For hdfs/path/{cluster}/{attribute} with input query "
          + "hdfs/path/" + cluster + "?limit=" + limit + "&path=" + path
          + " fetched #number of HdfsStats " + hdfsStats.size() + " in "
          + timer);
    } else {
      LOG.info("For hdfs/path/{cluster}/{attribute} with input query "
          + "hdfs/path/" + cluster + "?limit=" + limit + "&path=" + path
  ...
  public List<AppSummary> getNewJobs(@PathParam("cluster") String cluster,
      @QueryParam("user") String user, @QueryParam("startTime") long startTime,
      @QueryParam("endTime") long endTime, @QueryParam("limit") int limit)
      throws IOException {
    Stopwatch timer = new Stopwatch().start();
    ...
    AppSummaryService as = new AppSummaryService(HBASE_CONNECTION);
    // get the row keys from AppVersions table via JobHistoryService
    JobHistoryService jobHistoryService =
        new JobHistoryService(HBASE_CONF, HBASE_CONNECTION);
    List<AppSummary> newApps =
        as.getNewApps(jobHistoryService, StringUtils.trimToEmpty(cluster),
            StringUtils.trimToEmpty(user), startTime, endTime, limit);

    timer.stop();

    LOG.info("For newJobs/{cluster}/{user}/{appId}/ with input query "
        + "newJobs/" + cluster + SLASH + user + "?limit=" + limit
        + "&startTime=" + startTime + "&endTime=" + endTime + " fetched "
  ...
  public List<AppSummary> getAllApps(@PathParam("cluster") String cluster,
    ...
    LOG.info("Fetching all apps for cluster=" + cluster + " user=" + user
        + " startTime=" + startTime + " endTime=" + endTime);
    AppSummaryService as = new AppSummaryService(HBASE_CONNECTION);
    List<AppSummary> newApps = as.getAllApps(StringUtils.trimToEmpty(cluster),
        StringUtils.trimToEmpty(user), startTime, endTime, limit);
    timer.stop();
    LOG.info("For summary/apps/{cluster}/{user}/{appId}/ with input query "
        + "summary/apps/" + cluster + SLASH + user + "?limit=" + limit
        + "&startTime=" + startTime + "&endTime=" + endTime + " fetched "
