...
public class HRavenRestClient {
  ...
  /**
   * Initializes with the given hostname and a default connect and read timeout
   * of 5 seconds.
  ...
  public HRavenRestClient(String apiHostname, int connectTimeout,
      int readTimeout) {
  ...
  public String getCluster(String hostname) throws IOException {
    String urlString = String.format("http://%s/api/v1/getCluster?hostname=%s",
        apiHostname, StringUtil.cleanseToken(hostname));
  ...
  public List<Flow> fetchFlows(String cluster, String username,
      String batchDesc, String signature, int limit) throws IOException {
    LOG.info(String.format(
    ...
    String urlString = signature == null
        ? String.format("http://%s/api/v1/flow/%s/%s/%s?limit=%d", apiHostname,
            cluster, username, StringUtil.cleanseToken(batchDesc), limit)
  ...
  public List<Flow> fetchFlows(String cluster, String username,
      String batchDesc, String signature, List<String> flowResponseFilters,
      List<String> jobResponseFilters, int limit) throws IOException {
    LOG.info(String.format(
        "Fetching last %d matching jobs for cluster=%s, user.name=%s, "
            + "batch.desc=%s, pig.logical.plan.signature=%s",
        limit, cluster, username, batchDesc, signature));
  ...
  public List<Flow> fetchFlowsWithConfig(String cluster, String username,
    ...
    if ((configPropertyFields != null) && (configPropertyFields.size() > 0)) {
      ...
      urlStringBuilder
          .append(StringUtil.buildParam("includeConf", configPropertyFields));
    }
    return retrieveFlowsFromURL(urlStringBuilder.toString());
  }

  /**
   * Returns a list of flows that contain config elements matching a specific
   * pattern.
   * @param cluster
   * @param username
   * @param batchDesc
  ...
  public List<Flow> fetchFlowsWithConfigPatterns(String cluster,
      String username, String batchDesc, String signature, int limit,
  ...
  public List<Flow> fetchFlowsWithConfigPatterns(String cluster,
      String username, String batchDesc, String signature, int limit,
      List<String> flowResponseFilters, List<String> jobResponseFilters,
      List<String> configPatterns) throws IOException {
    LOG.info(String.format(
        "Fetching last %d matching jobs for cluster=%s, user.name=%s, "
            + "batch.desc=%s, pig.logical.plan.signature=%s",
        limit, cluster, username, batchDesc, signature));
    ...
    if ((configPatterns != null) && (configPatterns.size() > 0)) {
      urlStringBuilder.append(AND);
      urlStringBuilder
          .append(StringUtil.buildParam("includeConfRegex", configPatterns));
    }
    return retrieveFlowsFromURL(urlStringBuilder.toString());
  }
   ...
   * @throws IOException
   */
  private StringBuilder buildFlowURL(String cluster, String username,
      String batchDesc, String signature, int limit,
      List<String> flowResponseFilters, List<String> jobResponseFilters)
      throws IOException {
    ...
    if ((flowResponseFilters != null) && (flowResponseFilters.size() > 0)) {
      urlStringBuilder.append(AND);
      urlStringBuilder
          .append(StringUtil.buildParam("include", flowResponseFilters));
    }

    if ((jobResponseFilters != null) && (jobResponseFilters.size() > 0)) {
      urlStringBuilder.append(AND);
      urlStringBuilder
          .append(StringUtil.buildParam("includeJobField", jobResponseFilters));
    }

    return urlStringBuilder;
  }

  private List<Flow> retrieveFlowsFromURL(String endpointURL)
      throws IOException {
    if (LOG.isInfoEnabled()) {
      LOG.info("Requesting job history from " + endpointURL);
    }
    return new UrlDataLoader<Flow>(endpointURL,
        new TypeReference<List<Flow>>() {
        }, connectTimeout, readTimeout).load();
  }
  ...
  private List<TaskDetails> retrieveTaskDetailsFromUrl(String endpointURL) throws IOException {
    ...
    return new UrlDataLoader<TaskDetails>(endpointURL,
  ...
  public static void main(String[] args) throws IOException {
    ...
    if (apiHostname == null || cluster == null || username == null
        || batchDesc == null) {
      System.err.println(usage.toString());
      System.exit(1);
    }
    ...
    if (useHBaseAPI) {
      ...
      try {
        ...
        JobHistoryService jobHistoryService =
            new JobHistoryService(hbaseConfiguration, connection);
    ...
    for (Flow flow : flows) {
      ...
      if (minSubmitTime > 0 && maxFinishTime > 0) {
        System.out.println(String.format("Flow #%d: %s - %s", i++,
            DATE_FORMAT.format(minSubmitTime),
      ...
      for (JobDetails job : flow.getJobs()) {
        ...
        long reduceBytesRead = job.getReduceCounters()
            .getCounter("FileSystemCounters", "FILE_BYTES_READ") != null
                ? job.getReduceCounters()
                    .getCounter("FileSystemCounters", "FILE_BYTES_READ")
                    .getValue()
                : -1;
